...
const Configurator = () => {
  ...
  const loadJersey = (canvas, imageUrl) => {
    fabric.Image.fromURL(imageUrl, (img) => {
      ...
      img.set({
        ...
        top: (canvas.height - img.height * scaleFactor) / 2 - 30, // Поднимаем вверх на 30px
        selectable: true, // Если разрешено выделение для редактирования
        isJersey: true,   // Помечаем этот объект как веломайку
      });
      canvas.add(img);
  ...
  return (
    <div className="configurator-container">
      ...
      {fabricCanvas.current && <Toolbar canvas={fabricCanvas.current} />}
      {fabricCanvas.current && <LayerManager canvas={fabricCanvas.current} />}
